...
export default function ScanPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const copyNextConfig = () => {
    ...
    const needsCsp = headerFindings.some((f) => f.title.toLowerCase().includes("content-security-policy"));
    const needsHsts = headerFindings.some((f) => f.title.toLowerCase().includes("strict-transport"));
    const needsXfo = headerFindings.some((f) => f.title.toLowerCase().includes("frame") || f.module === "Clickjacking");
    const needsXcto = headerFindings.some((f) => f.title.toLowerCase().includes("content-type-options"));
    const needsRp = headerFindings.some((f) => f.title.toLowerCase().includes("referrer"));
    const needsPp = headerFindings.some((f) => f.title.toLowerCase().includes("permissions"));

    const headers: string[] = [];
    if (needsHsts) headers.push(`      { key: "Strict-Transport-Security", value: "max-age=63072000; includeSubDomains; preload" }`);
    if (needsXfo) headers.push(`      { key: "X-Frame-Options", value: "DENY" }`);
    if (needsXcto) headers.push(`      { key: "X-Content-Type-Options", value: "nosniff" }`);
    if (needsRp) headers.push(`      { key: "Referrer-Policy", value: "strict-origin-when-cross-origin" }`);
    if (needsPp) headers.push(`      { key: "Permissions-Policy", value: "camera=(), microphone=(), geolocation=()" }`);
    if (needsCsp) headers.push(`      { key: "Content-Security-Policy", value: "default-src 'self'; script-src 'self' 'unsafe-inline' 'unsafe-eval'; style-src 'self' 'unsafe-inline'; img-src 'self' data: https:; font-src 'self' data:; connect-src 'self' https:; frame-ancestors 'none'" }`);
    headers.push(`      { key: "X-DNS-Prefetch-Control", value: "on" }`);

    const config = `// next.config.ts — Security headers generated by VibeShield
// Based on scan of ${scan.target} (Grade: ${scan.grade}, Score: ${scan.score}/100)
              ...
              <div className="flex items-center gap-2 shrink-0">
                ...
                <div className="relative group/export">
                  ...
                  <div className="absolute right-0 top-full mt-1 bg-zinc-900 border border-zinc-800 rounded-lg py-1 min-w-[140px] opacity-0 invisible group-hover/export:opacity-100 group-hover/export:visible transition-all z-50 shadow-xl">
                    ...
                    <a href={`/api/scan/${id}/remediation`} target="_blank" rel="noopener" className="block text-xs text-zinc-400 hover:bg-zinc-800 hover:text-zinc-200 px-3 py-1.5 transition-colors">Remediation Plan</a>
